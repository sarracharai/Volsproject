...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		http.csrf().disable().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS).and().cors()
				...
				// consulter un vol par son id
				.requestMatchers("/api/getbyid/**").hasAnyAuthority("ADMIN", "USER")
				// ajouter un nouveau vol
				.requestMatchers(HttpMethod.POST, "/api/addv/**").hasAnyAuthority("ADMIN")
				// modifier un vol
				.requestMatchers(HttpMethod.PUT, "/api/updatev/**").hasAuthority("ADMIN")
				// supprimer un vol
				.requestMatchers(HttpMethod.DELETE, "/api/delv/**").hasAuthority("ADMIN").anyRequest().authenticated()
